	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(search!=null){
			if(search.equals("All")||search.equals("all")||search.equals("ALL")){
				...
				Collection<Quiz>quiz=quizService.findManyByName(search);
				System.out.println(quiz.size());
				request.setAttribute("quizzes", quiz);
				request.setAttribute("task", "search");
				request.setAttribute("results", resultService.findAll());
			...
			if(quizParam!=null){
				Quiz qz=quizService.findByName(quizParam);
				...
				if(uname!=null){
					...
					for(Result r:results){
						if(r.getQuiz()!=null&&qz.getQuizName().equals(r.getQuiz().getQuizName())){
							find=true;
							break;
						}
					}
					if(find){
						...
						if(qz!=null){
							...
							request.setAttribute("unanswered", null);
						}
					}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(uname==null){
			...
			if(pointsAjax!=null&&questionAjax!=null&&quizAjax!=null){
				...
				Collection<Result>find=q.getResults();
				
				User u=userService.findByUsername(uname);
				result.setTotalPoints(Integer.parseInt(pointsAjax));
				System.out.println(q.getQuizName()+"RESULT");
				...
				Result res=resultService.findByEmail(email);
				for(Result r : find){
					if(r.getResultHash().equals(res.getResultHash())){
						update=true;
					}
				}
				System.out.println(update);
				if(update){
					resultService.update(res, result);
				}
				else{
				...
				Result r=resultService.findByEmail(result.getEmail());
